...
MAX_MESSAGES_TO_SCAN = int(os.getenv("MAX_MESSAGES_TO_SCAN", "10000"))
CUTOFF_YEAR = 1955  # Solo películas anteriores a 1955 (< 1955)
...
def load_state() -> dict:
    ...
    return {
        "dest_chat_id": -1004388643765,
        "dest_invite_link": "https://t.me/+MJLcOPqptq05YTJj",
        "forwarded_titles": [],
        "forwarded_msg_ids": [],
        "total_synced": 0,
...
def extract_movie_year(raw_text: str) -> Optional[int]:
    """
    Extrae el año de producción de una película.
    """
    if not raw_text:
        return None

    # 1. Metadatos explícitos: "Año: 1942", "Estreno: 1939", "Year: 1950"
    ...
    # 2. Limpiar resoluciones y códecs
...
def normalize_movie_title(raw_title: str, caption: str = "") -> str:
    """
    Genera una clave normalizada de película (sin año ni tags de ripeo) para desduplicar.
    """
    cand = ""
    if caption:
...
def is_target_movie_video(message) -> Tuple[bool, str]:
    ...
    Excluye automáticamente archivos comprimidos (.rar, .zip, etc.).
    """
    ...
    # Ignorar archivos comprimidos
    if fname_lower.endswith((".rar", ".zip", ".7z", ".tar", ".gz")):
        return False, fname

    # Debe terminar en .mkv o .mp4
...
async def sync_peliculas_clasicas():
    ...
    async with TelegramClient(StringSession(STRING_SESSION), API_ID, API_HASH) as client:
        logger.info("=" * 65)
        logger.info("SINCRONIZACIÓN EN TIEMPO REAL - PELÍCULAS CLÁSICAS (< 1955)")
        ...
        # Punto de partida del escaneo (descendente: desde lo más nuevo hacia atrás)
        ...
        async for msg in client.iter_messages(source_chat, **iter_kwargs):
            ...
            if scanned_count % 1000 == 0:
                logger.info(f"🔍 [Progreso escaneo] {scanned_count} mensajes analizados (Msg ID actual: {msg.id}) | Clásicas reenviadas en esta tanda: {forwarded_in_run}")
            ...
            # 5. ¡REENVÍO INMEDIATO EN TIEMPO REAL!
            ...
            logger.info(f"🎬 [{forwarded_in_run + 1}/{BATCH_FORWARD_LIMIT}] Reenviando al grupo: {display_title} ({year}) | {file_size_mb:.1f} MB (Msg ID: {msg.id})...")

            sent_success = False
            for attempt in range(3):
                try:
                    # Intento 1: Copia limpia nativa en la nube (sin etiqueta de 'Reenviado de')
                    ...
                    logger.warning(f"Aviso send_message ({e}), recurriendo a forward_messages...")
                    try:
            ...
            if sent_success:
                ...
                save_state(state)
                logger.info(f"✅ ¡Película entregada en el grupo!: '{display_title}' ({year})")

                # Pausa de cortesía para no saturar la cuenta de Telegram
                await asyncio.sleep(random.uniform(2.5, 3.5))

            # Si alcanzamos el cupo del lote para esta ejecución
            if forwarded_in_run >= BATCH_FORWARD_LIMIT:
                logger.info(f"🎯 Límite de lote alcanzado ({BATCH_FORWARD_LIMIT} películas reenviadas).")
                break

            # O si escaneamos demasiados mensajes sin encontrar suficientes
            if scanned_count >= MAX_MESSAGES_TO_SCAN:
                logger.info(f"Escaneados {scanned_count} mensajes en este ciclo.")
                break

        # Guardar último ID escaneado para continuar en la siguiente ejecución
        ...
        logger.info(f"  • Películas clásicas reenviadas en esta tanda: {forwarded_in_run}")
        logger.info(f"  • Total acumulado en 'Peliculas Clasicas': {state['total_synced']}")
        logger.info(f"  • Siguiente ID a escanear: {state['last_message_id_scanned']}")
        logger.info("=" * 65)
        ...
        if current_lowest_id and current_lowest_id > 500335:
            logger.info(f"Aún queda catálogo histórico por explorar. Creando marcador '{MORE_MARKER}'...")
            with open(MORE_MARKER, "w") as f:
                f.write(str(current_lowest_id))
        else:
            logger.info("🎉 ¡Se ha explorado la totalidad del catálogo del Tema 7!")
